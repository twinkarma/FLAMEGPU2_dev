...
xmlReader::xmlReader(const std::string &model_name, const unsigned int &instance_id, const std::unordered_map<std::string, std::shared_ptr<AgentPopulation>> &model_state, const std::string &input)
    : StateReader(model_name, instance_id, model_state, input) {}
...
int xmlReader::parse() {
    ...
    if (pElement) {
        for (auto envElement = pElement->FirstChildElement(); envElement; envElement = envElement->NextSiblingElement()) {
            ...
            std::string token;
            const EnvironmentManager::NamePair np = { instance_id , std::string(key) };
